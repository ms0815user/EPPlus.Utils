...
namespace EPPlus.Utils.src
{
    ...
    public static class ColumnAndRow
    {
        ...
        private static void ExpandValue(this ExcelRangeBase range, IList valuesToInsert, int[] index, int mode, InsertMode Mode, int cellsToExpand = 0, IList expandValues = null)
        {
            for (var i = valuesToInsert.Count - 1; i >= 0; i--)
            {
                ...
                for (var j = 0; j < Math.Abs(cellsToExpand); j++)
                {
                    try
                    {
                        range.Worksheet.SetValue(row + (1 + j) * colIndicator, col + (1 + j) * rowIndicator, expandValues?[j + i * Math.Abs(cellsToExpand)]); // You must set value to newly inserted cells, otherwise the sheet can't get its range address
                    }
                    catch (Exception e)
                    {
        ...
        {
            var rowIndex = range.GetIndex(valuesToInsert, Mode, out var modeValue, columnsToExpand, expandValues);
        ...
        {
            var columnIndex = range.GetIndex(valuesToInsert, Mode, out var modeValue, rowsToExpand, expandValues);
